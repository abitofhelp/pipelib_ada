...
package Pipelib.Core.Domain.Value_Objects.Stage_Order is
   ...
   function Is_First (Self : Stage_Order_Type) return Boolean
   is (Value (Self) = 1);

   --  Calculate distance between two orders
   function Distance (From, To : Stage_Order_Type) return Natural
   is (if Value (To) >= Value (From) then Value (To) - Value (From)
       else Value (From) - Value (To));

   --  Comparison operators
   function "<" (Left, Right : Stage_Order_Type) return Boolean
   ...
   function Image (Self : Stage_Order_Type) return String
   is ("Stage Order: " & Value (Self)'Image);
   ...
   with
     Post =>
       Create_Sequence'Result'Length = Count
       and then (for all I in Create_Sequence'Result'Range
...
private
   ...
   function Is_Valid (Self : Stage_Order_Type) return Boolean
   is (True);  -- Always valid since Order_Value is Positive
