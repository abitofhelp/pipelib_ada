...
package Pipelib.Core.Domain.Value_Objects.Worker_Count is
   ...
   function Create (Value : Natural) return Worker_Count_Type
     with Pre => Value > 0 and then Value <= 256,
   ...
   function Get_CPU_Based_Count (Multiplier : Positive := 1) return Worker_Count_Type
     with Pre => Multiplier <= 32,
          Post => Get_CPU_Based_Count'Result >= 1 and then
                  Get_CPU_Based_Count'Result <= 256;

   --  Get optimal worker count based on file size (empirically optimized)
   function Optimal_For_File_Size (File_Size : Long_Long_Integer) return Worker_Count_Type
     with Post => Optimal_For_File_Size'Result >= 1 and then
                  Optimal_For_File_Size'Result <= 256;
   ...
   function Optimal_For_Processing_Type
     ...
     with Pre => Available_CPUs > 0,
          Post => Optimal_For_Processing_Type'Result >= 1 and then
                  Optimal_For_Processing_Type'Result <= 256;

   --  Image function for debugging
   function Image (Count : Worker_Count_Type) return String is
     (Count'Image);
