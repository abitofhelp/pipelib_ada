...
generic
   ...
   with
     function Input_To_Stage_Data (Input : Input_Type) return Stage_Data_Type;
   with
     function Stage_Data_To_Output (Data : Stage_Data_Type) return Output_Type;
...
package Pipelib.Core.Application.Services.Generic_Pipeline_Processor
is
   ...
   function Process_Item
     (Stage : Stage_Interface; Item : in out Stage_Data_Type)
   ...
   with
     Post =>
       Stage_Count (Create'Result) = 0
       and then not Is_Processing (Create'Result);
   ...
   with
     Pre => not Is_Processing (Processor) and then Has_Stage (Processor, Name),
     Post => Stage_Count (Processor) = Stage_Count (Processor'Old) - 1;
   ...
   with
     Pre =>
       Stage_Count (Processor) > 0
       and then not Is_Processing (Processor)
       and then Is_Valid_Input (Input),
     Post =>
       not Is_Processing (Processor)
       and then (if Output_Result.Is_Ok (Process'Result)
                 then Is_Valid_Output (Output_Result.Get_Ok (Process'Result)));
   ...
   with
     Pre => Stage_Count (Processor) > 0 and then not Is_Processing (Processor),
     Post => Is_Initialized (Processor);

   procedure Finalize_Pipeline (Processor : in out Pipeline_Processor)
   with
     Pre => Is_Initialized (Processor) and then not Is_Processing (Processor),
   ...
   procedure Set_Error_Handler
     (Processor : in out Pipeline_Processor; Handler : Error_Handler_Access)
